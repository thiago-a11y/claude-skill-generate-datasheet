...
> Este documento é o ponto de entrada para qualquer IA, LLM, ou humano que precise
> entender o projeto completo. Cole este arquivo em qualquer chat com Claude, GPT,
> Gemini, ou outra IA para obter feedback, ideias, e sugestões informadas.
>
...
## What is this project? / O que é este projeto?
...
An offline Python CLI tool that does the same scanning but **without any AI, internet,
...
- **Creator**: Thiago Xavier — Objetiva Solução Empresarial (Brazil)
- **Contact**: thiago@objetivasolucao.com.br
- **GitHub**: https://github.com/thiago-a11y/claude-skill-generate-datasheet
- **Built in**: 5 days (May 27-31, 2026), 19 commits, with Claude Code assistance
...
│                            │                                │
│   skills/                  │   codedocs/                    │
│   ├─ generate-datasheet/   │   ├─ scanner.py  (13 scans)   │
│   ├─ generate-api-client/  │   ├─ renderer.py (4 HTMLs)    │
│   ├─ generate-compliance/  │   ├─ migration.py (planner)   │
│   └─ health-badges/        │   └─ cli.py     (interface)   │
│                            │                                │
│   Output: 30+ MD files,    │   Output: 4 HTML files         │
│   2 HTML files, fixes      │   (scan report, sales,         │
│   applied with approval    │   tech spec, migration plan)   │
├────────────────────────────┴────────────────────────────────┤
│   docs/           — 7 Perplexity research + 10 generated    │
...
| 5 — Operational Intelligence | Role-based onboarding (4 roles), bus-factor report, incident runbooks, health score 0-100 | Ops, SRE, product |
...
```bash
python3 -m codedocs /path/to/project                    # basic scan
python3 -m codedocs /path --migration --target react+fastapi --erp SAP TOTVS  # with migration plan
```

### Scanner (scanner.py — 13 scan functions)
...
### Migration Planner (migration.py)
...
### Renderer (renderer.py — 4 HTML generators)
Generates self-contained HTML files (inline CSS, dark theme, print-friendly):
1. Scan Report — full inventory with health score
2. Sales Datasheet — metrics bar, modules, honest limitations
3. Technical Spec — "6 answers in 60 seconds", security matrix
4. Migration Plan — target selector, equivalence map, blockers, phased roadmap, ERP plans
...
7 Perplexity research sessions documented in `docs/`:
...
### What's done / O que está feito
- 4 Claude Code skills (generate-datasheet v4.1, generate-api-client v1.0, generate-compliance v1.0, health-badges v1.0)
- CodeDocs v1.3.1 (offline CLI with full migration planner)
- 7 research docs + 17 generated docs
- Full documentation pack generated (this session)
- 2 correction fixes applied (Layer 6)

### Health Score: 42/100
| Dimension | Score | Why |
|-----------|-------|-----|
| Test Coverage | 0/100 | 0 test files for 2700 LOC |
| Security Posture | 90/100 | Zero deps, zero network, HTML escaping |
| Tech Debt | 95/100 | 0 TODOs in source code |
| Documentation | 100/100 | 17+ markdown files |
...
3. **Skills not tested on external codebases** — only tested on this repo
...
3. Add pytest smoke tests for CodeDocs scanner + renderer
4. Create GitHub Actions workflow (lint + test)
5. Tag releases (v4.1.0 for skills, v1.3.1 for CodeDocs)
...
- **Try `python3 -m codedocs /your/project`** and report the results
